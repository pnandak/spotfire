...
## 2. You should write/submit R codes in script editor, and save your script freq.ly.
...
## 2. Only '.', '_' and alphanumeric symbols 'a-zA-Z0-9' allowed.
## 3. The name of an object must begin with a letter or '.' then letter.
...
## Google 'R zero inflated poisson regression' 
...
# Elements in a vector can ONLY be of ONE value type, namely: numeric, logical 
## or character. 
...
## Consider the following Australian teacher salary data
...
# A vector is 1-dim. A matrix is 2-dim, a special case of array. 
## An array can be multi-dimesional.
...
## vectors v.n, v.s, v.b of length 4.
    ...
    df.1 = data.frame(v.n,v.s,v.b) #df.1 appears like a matrix, but it's not.
    m.5 = cbind(v.n,v.s,v.b)

# Data frame is the data structure in R for data imported from external files through the
## function 'read.table()'.

## Get the path of current working directory for R
    getwd() 
## It may be convenient to set working directory to where your data file is stored.
    setwd('C:\\Documents and Settings\\Sai\\Desktop\\R_intro') # use '\\' or '/'
## Now load 'dat1_houses.txt' file
    df.2 = read.table('dat1_houses.txt',header=T)
...
## You can also use the way of referencing values in a matrix. 
    df.2[1,] # 1st row; 
...
## 'apply()' is useful if you want a function to operate along a specifc dimension.
    apply(m.5,2,mean) # mean for each column
...
# First run the following codes to make arbitrary data 
    ...
    x = runif(n,0,4) 
    e = rnorm(n,0,.2)
    e[50] = e[50]+1.5 # add an outlier at the 50th obs 
