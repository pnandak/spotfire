...
#   4) Biasedness and Consistency - an example
...
read.csv("C:/cancer.csv", header=FALSE)
cancer <- read.csv("C:/cancer.csv", header=FALSE)
...
idx <- sample(1:length(sunspots), size, replace=FALSE)
idx

#   You can then extract only the observations that you want and calculate
#   the relevant sample statistics:

sample.mean <- mean(sunspots[idx])
sample.var <- var(sunspots[idx])
...
for(ii in 1:10000){
idx <- sample(1:length(sunspots), size, replace=FALSE)
draw.mean[ii] <- mean(sunspots[idx])
draw.var[ii] <- var(sunspots[idx])
}
...
#    Mean squared error is a measure of the efficiency of an estimator.  It is 
#    the expected value of the squared differences between the estimator and the
#    true value of the parameter that it is being used to estimate.  In the class
#    of unbiased estimators for a parameter, we are often interested in finding 
#    the most efficient unbiased estimator.

#    Estimating the mean squared error of an estimator in a particular case is 
#    easy through simulation.  For each draw, we want to calculate the difference
#    between our estimate of the parameter and the true value:

estimation.error <- draw.mean - mean.sunspots
...
mse.sd <- mean((sqrt(draw.var) - sqrt(var.sunspots))^2)
mse.sd
...
size <- 36
draws.max.36 <- numeric()
for(ii in 1:1000){
idx <- sample(1:length(sunspots), size, replace=FALSE)
draws.max.36[ii] <- max(sunspots[idx])
}
...
size <- 100
draws.max.100 <- numeric()
for(ii in 1:1000){
idx <- sample(1:length(sunspots), size, replace=FALSE)
draws.max.100[ii] <- max(sunspots[idx])
}
...
size <- 1000
draws.max.1000 <- numeric()
for(ii in 1:1000){
idx <- sample(1:length(sunspots), size, replace=FALSE)
draws.max.1000[ii] <- max(sunspots[idx])
}
...
size <- obs - 1
draws.max.obs <- numeric()
for(ii in 1:10000){
idx <- sample(1:length(sunspots), size, replace=FALSE)
draws.max.obs[ii] <- max(sunspots[idx])
}
