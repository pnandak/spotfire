...
##  H_A : mu not equal to 50

p.value <- pnorm(-t.stat, 0, 1) + 1 - pnorm(t.stat, 0, 1)
p.value

##  H_A : mu greater than 50
...
xx1 <- rnorm(100, mean = 4, sd = 5)
xx2 <- rnorm(200, mean = 5, sd = 5)
...
##  Now, what if we have data where the variances are not equal:

yy1 <- rnorm(20, mean = 4, sd = 5)
yy2 <- rnorm(10, mean = 5, sd = 10)

t.test(yy1, yy2, alternative = "two.sided", var.equal = TRUE)
t.test(yy1, yy2, alternative = "two.sided", var.equal = FALSE)

##  In this case, assuming that they are equal when their are not actually 
##  equal tends to produce test statistics that are too high, so you are 
##  making more Type I errors than you think you are.