##
##  section8.R - interaction term goodness
##  mb - 12 Nov 08
##   
...
## what is the expected length of my hangover if 
## I drink 5 beers and have water before bed?
## what if I drink 5 beers and have no water?
...
## make some data (ignore this):       
product <- 30 + 5*beers - .95*beers^2 + rnorm(100,0,1.5)
product

lin.model <- lm(product ~ beers)
summary(lin.model)

quad.model <- lm(product ~ beers + beers^2)
summary(quad.model)

quad.model <- lm(product ~ beers + I(beers^2))
summary(quad.model) 


## in the past we've able to plot with abline() but it's clear
## that now we will have to plot a parabola. how do we do this?
## using the curve() function. 

plot(x=beers, y=product, pch=19, col="chocolate")
abline(coef=coef(lin.model), col="chocolate")

curve(coef(quad.model)["(Intercept)"] + 
      coef(quad.model)["beers"] * x +
      coef(quad.model)["I(beers^2)"] * x^2, col="chocolate")
      

## bah, what happened? curve() needs to be told to add the curve to
## an existing plot. 
...
curve(coef(quad.model)["(Intercept)"] + 
      coef(quad.model)["beers"] * x +
      coef(quad.model)["I(beers^2)"] * x^2, col="chocolate", add=T)