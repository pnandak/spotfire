# rgdal, maptools, ipred, gstat, fields
#
# sp, mlbench, gpclib
# rpart, MASS, survival, splines, nnet, class, foreign, 
# 
# BMcD.shp, BMcD.shx, BMcD.dbf, BMcD.prj, BMcD_fldf.txt
#
###################################################
library(rgdal)
BMcD <- readOGR(".", "BMcD")
...
tull <- by(as(BMcD, "data.frame"), BMcD$Fldf, function(x)
  plot(density(x$Zn), main="", xlim=c(0,2000), lwd=2))
par(oopar)
...
res <- errorest(Zn ~ 1, data = as(BMcD, "data.frame"), model=lm,
  est.para=control.errorest(k=nrow(BMcD), random=FALSE, predictions=TRUE))
round(res$error, 2)
...
symbols(coordinates(BMcD), circles=sqrt(abs(pe)), fg="black",
...
symbols(coordinates(BMcD), circles=sqrt(abs(pe_tps)), fg="black",
  bg=scols[(pe_tps < 0)+1], inches=FALSE, add=TRUE)
legend("topleft", fill=cols, legend=leglabs(brks), bty="n", cex=0.8)
...
symbols(coordinates(BMcD), circles=sqrt(abs(pe_UK)), fg="black",
  bg=scols[(pe_UK < 0)+1], inches=FALSE, add=TRUE)
legend("topleft", fill=cols, legend=leglabs(brks), bty="n", cex=0.8)
...
spplot(BMcD_grid, c("lm_pred", "spl_pred", "OK_pred", "UK_pred"),
  at=brks, col.regions=cols, sp.layout=list(pts))
