# rgdal, maptools, classInt, RColorBrewer, spdep, DCluster and dependencies
# (sp, tripack, SparseM, e1071) : (MASS, foreign, boot, class)
#
# scot.dbf, scot.shp, scot.shx, scotland.dat, CK.gal
#
###################################################
library(rgdal)
...
title(main="Observed")
legend("topleft", fill=attr(O_CIc, "palette"),
 legend=names(attr(O_CIc, "table")), bty="n")
...
title(main="Expected")
legend("topleft", fill=attr(E_CIc, "palette"),
 legend=names(attr(E_CIc, "table")), bty="n")
...
legend("topleft", fill=c("grey35", "white", "grey75"),
 legend=c("low", "N/S", "high"), bty="n")
...
legend("topleft", fill=attr(cols, "palette"), legend=legtext, bty="n",
 cex=0.7, y.inter=0.8)
...
###################################################
spplot(scot_BNG, c("SMR", "EB_mm", "EB_ml", "EB_mm_local"),
 col.regions=bluepal(16))


###################################################
lw <- nb2listw(CK_nb)
set.seed(20060614)
moran.boot <- boot(as(scot_BNG, "data.frame"), statistic=moranI.boot,
 R=999, listw=lw, n=length(CK_nb), S0=Szero(lw))
...
###################################################
moran.pgboot <- boot(as(scot_BNG, "data.frame"), statistic=moranI.pboot,
 sim="parametric", ran.gen=negbin.sim, R=999, listw=lw, n=length(CK_nb),
 S0=Szero(lw))
...
###################################################
writePolyShape(scot_BNG, "scot_BNG_new")
crs <- showWKT(proj4string(scot_BNG), "scot_BNG_new.prj")
