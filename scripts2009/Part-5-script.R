##--------------------------------------------##
## Script for Part 5: Graphics Introduction   ##
##    John Fox                                ##
...
plot(z, p, type="l", lwd=2,
    ...
    ylab=expression(phi(z) == frac(1, sqrt(2*pi)) * ~~ e^- ~~ frac(z^2,
         2)))
abline(h=0, col="gray")
abline(v=0, col="gray")
...
UN <- read.table(
  "http://socserv.socsci.mcmaster.ca/jfox/Courses/UCLA/UnitedNations.txt")
...
plot(range(gdp), c(0,1), xlab="GDP per Capita", ylab="Tricube Kernel Weight", 
