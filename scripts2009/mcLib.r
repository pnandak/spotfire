...
cat("<div>
    ...
    /* Heatmap Styles */
        ...
        .mc-heatmap td, .mc-heatmap th {
                border-right: 1px solid black;
                border-bottom: 1px solid black;
        }
    .mc-heatmap th.hm-row-title {
        text-align: right;
        font-weight: bold;
        color: #000000;
    }
    .mc-heatmap th.hm-col-title {
        text-align: center;
        font-weight: bold;
        color: #000000;
    }
    ...
    /* 95% Confidence */
    .mc-heatmap td.conf95 {
        color: #000000;
        background-color: #B4EEB4;
        font-size: larger;
    }
    /* 99% Confidence */
    .mc-heatmap td.conf99 {
        color: #000000;
        background-color: #33FF33;
        font-weight: bold;
        font-size: larger;
    ...
    <table class=\"mc-heatmap\">
      <tr>
         <th scope=\"col\" class=\"hm-col-title\"></th>", file=htmlFileName, append=FALSE, sep="")
for (j in (1:ncol(values))) {
  cat("
         <th scope=\"col\" class=\"hm-col-title\">",colnames(values)[j],"</th>",file=htmlFileName, append=TRUE, sep="")
}
cat("
...
cat("
</div>",file=htmlFileName, append=TRUE, sep="")
}
...
#     metricName is the name of the metric
#     htmlFileName is the absolute path name of the file to write the HTML
#     values is the matrix of values for the table including headers
...
# **************************************************************************
top10PrintHTML<-function(metricName,htmlFileName,values,styles,evenColor,oddColor) { 
cat("<div>
    <style>
        table.mc-top10 { 
             width: 100%;
...
# ****************************************************************************************
# dataSetsUsed Function - Returns a table with statistics about a vp dataset
#        ds = a dataframe with the following columns
#             vendor
#             product
#             version
#             vulnID
#
# Only useful for getting stats on the raw dataset from NVD or OSVDB
# ****************************************************************************************
vpDataSetStats<-function(ds) { 
    # Number of rows
    rc<-dim(ds)[1]
    cc<-dim(ds)[2]
    nvulns<-length(levels(as.factor(ds$vulnID)))
    nvendors<-length(levels(as.factor(ds$vendor)))

    # Distribution:  Vulns Per Vendor-Product-Version
    tab<-paste(ds$vendor,ds$product,ds$version,sep="::")
    VulnsPerVPV<-table(tab)
    VulnsPerVendorProductVersion<-summary(as.numeric(VulnsPerVPV))
    ...
    VendorProductVersionsPerVuln<-summary(as.numeric(VPVPerVuln))
    ...
    ProductVersionsPerVendor<-summary(as.numeric(PVPerVendor))
    ...
    VersionsPerVendorProduct<-summary(as.numeric(VerPerVendorProd))
    ...
    ProductsPerVendor<-summary(as.numeric(ProdPerVendor))

    # Assemble into a values table
    Variable<-c("Min","1stQ","Median","Mean","3rdQ","Max")
    DSstats<-rbind(Variable,
                   VulnsPerVendorProductVersion,
                   VendorProductVersionsPerVuln,
                   ProductVersionsPerVendor,
                   VersionsPerVendorProduct,
                   ProductsPerVendor)
    DSstats<-cbind(rownames(DSstats),DSstats)
    return(list(rowCount=rc,colCount=cc,
                nVuln=nvulns,nVendor=nvendors,
                dsStats=DSstats))
}
# **************************************************************************
#  HTML vulnStats Table Function
# **************************************************************************
# dsStatsPrintHTML(dsName,dsDescr,rowCount,colCount,nVuln,nVend,values, htmlFileName,rowStyles,evenColor,oddColor)
#     dsName is the name of the dataset
#     dsDescr is a description of the dataset
#     rowCount is the number of rows
#     colCount is the number of columns
#     nVuln is number of vuln
#     nVend is number of vendors
#     values is the matrix of values for the table including text headers
#     htmlFileName is the absolute path name of the file to write the HTML
#     rowStyles is a vector styles for each row in the table
#     evenColor is the color for the background of even rows
#     oddColor is the color for the background of odd rows
# **************************************************************************
dsStatsPrintHTML<-function(dsName,dsDescr,rowCount,colCount,nVuln,nVend,values,
                  htmlFileName,styles,evenColor,oddColor) {
cat("<div>
    <style>
        table.mc-hdr {
             width: 100%;
        ...
        .mc-vulnStats td, .mc-vulnStats th {
                border: 0px none;
                border-right: 1px solid black;
                border-bottom: 1px solid black;
        }
        ...
        .mc-vulnStats tr.rowEven {
            background-color: ", evenColor, ";
            color: #000000;
        }
        .mc-vulnStats tr.rowOdd {
            background-color: ", oddColor, ";
            color: #000000;
        }
        .mc-vulnStats tr.rowTop {
            font-weight: bold;
            background-color: #E8E8E8;
            color: #000000;
        }
    </style>", file=htmlFileName, append=FALSE,seg="")
  cat("
    <table class=\"mc-hdr\">
       <tr>
         <th>DataSet:  </td>
       ...
       <tr>
         <th>Dim:  </td>
         <td>", rowCount," rows x ", colCount, "columns</td>
       </tr>
       <tr>
         <th>Data:  </td>
         <td>", nVend," vendors x ", nVuln, " vulnerabilities</td>
       </tr>
    </table>", file=htmlFileName, append=TRUE,seg="")
  cat("
    <table class=\"mc-vulnStats\" >
      <tr class=\"rowTop\">", file=htmlFileName, append=TRUE, sep="")
for (j in (1:ncol(values))) {
  cat("
         <th scope=\"col\">",values[1,j],"</th>",file=htmlFileName, append=TRUE, sep="")
}
