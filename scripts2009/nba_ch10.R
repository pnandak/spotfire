...
# Section 10.1
  ...
  library(car)
  av.plots(model = mod.fit) #Interactive
  av.plots(model = mod.fit,  ask=FALSE, one.page = TRUE, identify.points=FALSE) 
  ...
  #EXTRA
  mod.fit1<-lm(formula = PPM ~   height + FGP + age, data = nba)
  mod.fit2<-lm(formula = MPG ~   height + FGP + age, data = nba)
  plot(x = mod.fit2$residuals , y = mod.fit1$residuals)
  #matches av.plot()
