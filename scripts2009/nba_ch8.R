...
# 3D plots
  ...
  scatter3d(x = nba$age, y = nba$PPM, z = nba$MPG, 
          ...
          bg.col="black")
  scatter3d(x = nba$age, y = nba$PPM, z = nba$MPG, 
          fit=c("linear", "quadratic"), grid=TRUE, xlab="age", ylab="PPM", zlab="MPG", 
          bg.col="black")         
...
# Section 8.2 - interaction regression models
  ...
  save.xyz$PPM.hat<-predict(object = mod.fit.inter, newdata = save.xyz)


  win.graph(width = 8, height = 6, pointsize = 10)
  wireframe(x=PPM.hat ~ age + MPG, data = save.xyz, scales = list(arrows = FALSE), drape = TRUE, 
