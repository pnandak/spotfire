...
#Note from 8-15-08: I should have set a seed number so that I could have reproduced the exact results :(
...
# Method #1
  ...
  groups<-ifelse(test = mod.fit$fitted.values < quant5[1], yes = 1, no = 
  ...
  group.var<-ifelse(test = groups == 1, yes = var.eps[1], no = 
  ...
  #Try calculating a C.I. for X = 40
  pred1<-predict(object = mod.fit1, newdata = data.frame(X = 40), interval = "confidence", level = 0.95)
...
# Method #2
  ...
  #Try calculating a C.I. for X = 40
  pred2<-predict(object = mod.fit2, newdata = data.frame(X = 40), interval = "confidence", level = 0.95)
