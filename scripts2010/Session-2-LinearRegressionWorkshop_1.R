...
# If you don't have the car data set you can load the Robey data from 
# the web as follows.
data.url <-  "http://www.stat.wisc.edu/~neely/data/Robey.csv"
Robey <- read.csv(file=data.url,row.names=1)
...
# Fix the incorrect spelling of Mauritius.

rownames(Robey)[8] <- "Mauritius"
...
# Plot the raw data using the plot command.
plot(Robey$contraceptors,Robey$tfr,type='p')

# Plot the row names.
plot(Robey$contraceptors,Robey$tfr,type='n')
text(Robey$contraceptors,Robey$tfr,labels=row.names(Robey),cex=0.7)


# Now just plot the data for Africa by extracting a subset just containing the 
# data for Africa.

Africa.Robey <- subset(Robey,subset=region=='Africa')
plot(Africa.Robey$contraceptors,Africa.Robey$tfr,type='n')
text(Africa.Robey$contraceptors,Africa.Robey$tfr,
         labels=row.names(Africa.Robey),cex=0.7)
