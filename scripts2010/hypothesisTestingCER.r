# hypothesisTesting.r			R script for hypothesis testing examples
#
# author: Eric Zivot
...
smpl = positions(returns.z) >= start(roll.rhohat)

par(mfrow=c(2,1))
plot(returns.z[smpl,c("sp500","sbux")],reference.grid=F,
     main="Returns on sp500 and sbux",
     plot.args=list(lwd=c(2,2), col=c(2,5)))
abline(h=0)
plot(roll.rhohat,reference.grid=F,
     main="24 month rolling correlations b/w sp500 and sbux",
     plot.args=list(lwd=2))
par(mfrow=c(1,1))

# compute rolling correlations b/w sbux and msft
roll.rhohat = aggregateSeries(returns.z[,c("msft","sbux")],
                              moving=24,adj=1,together=T,
                              FUN=rhohat)
class(roll.rhohat)
roll.rhohat[1:5]

plot(roll.rhohat, lwd=2, col="blue", ylim=c(0,1), ylab="rhohat",
     main="24 month rolling correlations b/w sbux and sp500")
