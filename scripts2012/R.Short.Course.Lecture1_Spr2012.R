...
# ---------------------------------------------------------------------
# Revision History
#    -- Tue, Jan 10, 3:27 pm (added graphing stuff; added challenging questions)
...
#    only rows with TwinNum 2 and Zygosity "DZ".

# f) X is standardized height for TW1 and Y is stndzd height for TW2.
#    Let's say we're interested in knowing whether the two twin 
#    correlations (MZ and DZ) differ as a function of whether the mean 
#    twin height is above the overall mean in the sample. Can you find
#    the 4 correlations (rMZ & rDZ for those twin pairs > avg height,
#    and rMZ and rDZ for those twin paird < avg height). Challenging.
#    First find the avg height for each twin pair. Then select based
#    on that variable.
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# PART 8: LOOKING AT YOUR DATA
# ---------------------------------------------------------------------

# One of the joys of R is that it is very easy to visualize your data
# and there are thousands of high-level methods of doing so. 
# Let's start off with the basics, shall we?

# Look at a histogram of X1 in ExampleData1
hist(ExampleData1$X1)

# Let's change some of the options.
?hist

# Change the number of breaks, the title, the color, and the x label
colors()  #to get a vector of all your color options
hist(ExampleData1$X1,col='lightblue',main="My First Histogram in R",
     breaks=50,xlab="Standardized Height, Twin 1")

# Scatterplot of X1 vs Y1
plot(ExampleData1$X1,ExampleData1$Y1)

# Customize the graph above
plot(ExampleData1$X1,ExampleData1$Y1, col='green',main="twin scatterplot",xlab="twin 1 height",ylab="twin 2 height",pch=16)

# See the function "par" for many additional graphing options in R graphs
?par

# Many graphing functions *add* something to an already existing graph
# allowing you fine-tuned ability to customize your graphs. 
# Let's add a best fit line to the above
mod1 <- lm(ExampleData1$Y1~ExampleData1$X1)
coefs.mod1 <- coef(mod1)
abline(coef=coefs.mod1,col='blue',lwd=2)

#Scatterplot over the Histogram 
par()$mfrow        #the default for parameter "mfrow"
par(mfrow=c(2,1))  #says we want the plot split into 2 rows, 1 column

plot(ExampleData1$X2,ExampleData1$Y2, col='blue',main="twin scatterplot",xlab="twin 1 height",ylab="twin 2 height",pch=16)
abline(coef=coef(lm(ExampleData1$Y2~ExampleData1$X2)),col='red',lwd=2)

hist(ExampleData1$X2,col='lightblue',breaks=25,main='twin 1 height histogram',xlab='twin 1 height')

par(mfrow=c(1,1))  #sets back to the default
...
# ---------------------------------------------------------------------
#
# a) Using the data in ExampleData1.csv, create a set of 4 scatterplots, with 
#    the MZ correlations for X1,Y1 and then X2,Y2 on the top row and the DZ 
#    correlations for X1,Y1 and X2,Y2 on the bottom row. Add best-fit lines
#    to each one. Make the MZ datapoints a different color than the DZ
#    datapoints.

# b) Do a scatterplot of X1,Y1 in ExampleData2 for MZ twins. Use the "text"
#    function (?text) to add this text to your graph "rMZ = X", where X
#    is the MZ correlation (rounded to two digits so it looks nice).
